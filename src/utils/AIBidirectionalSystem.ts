...
import {
  ...
  assembleSystemPromptSections,
  sectionsToInjects,
  type PromptSection,
} from './prompts/promptAssembler'
...
import {
  isSaveDataV3,
  migrateSaveDataToLatest,
  migrateNarrativeCharacterState,
} from './saveMigration'
...
class AIBidirectionalSystemClass {
  ...
  private buildFocusedNpcPrompt(stateForAI: any): string {
    ...
    const list =
      focusedNames.length > 0 ? focusedNames.map((name) => `- ${name}`).join('\n') : '- （无）'
    return [
      '# 🔎 实时关注NPC（必须更新）',
  ...
  private normalizeEventConfig(config: any): {
    enabled: boolean
    minYears: number
    maxYears: number
    customPrompt: string
  ...
  private async maybeTriggerScheduledWorldEvent(args: {
    ...
    const { enabled, minYears, maxYears, customPrompt } = this.normalizeEventConfig(
      (eventSystem as any).配置,
    )
    if (!enabled) return
    ...
    try {
      const { generateWorldEvent, generateSpecialNpcEvent } =
        await import('@/utils/generators/eventGenerators')
      const gameStateStore = useGameStateStore()
      ...
      if (npcToAdd && npcToAdd.名字) {
        ...
        if (stateForAI?.社交) {
          if (!stateForAI.社交.关系 || typeof stateForAI.社交.关系 !== 'object')
            stateForAI.社交.关系 = {}
        ...
        const current =
          gameStateStore.relationships && typeof gameStateStore.relationships === 'object'
      ...
      if (!v3.社交) v3.社交 = {}
      if (!v3.社交.记忆 || typeof v3.社交.记忆 !== 'object')
        v3.社交.记忆 = { 短期记忆: [], 中期记忆: [], 长期记忆: [] }
  ...
  public async processPlayerAction(
    ...
    const generationId =
      options?.generation_id || `gen_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    ...
    try {
      const v3 = isSaveDataV3(saveData)
        ? (saveData as any)
        : migrateSaveDataToLatest(saveData).migrated
      ...
      try {
        ...
        if (
          vectorMemoryService.isEnabled() &&
          Array.isArray(longTermMemories) &&
          ...
          if (stats.total === 0) {
            ...
            console.log(
              `[向量记忆] 已注入 ${results.length} 条相关长期记忆（向量库总数：${stats.total}）`,
            )
          }
        }
      } catch (e) {
...
- 环境修正根据判定类型选择对应的值`
      ...
      const buildConciseNarrativeState = (): Record<string, unknown> => {
        const {
          ...
          人物属性,
          cleanedSocial,
        } = migrateNarrativeCharacterState(stateForAI)
      ...
      // 按角色分组的提示词段落（每个提示词可独立设置 system/user/assistant 角色）
      const promptSections = await assembleSystemPromptSections(
        activePrompts,
...
`
      ...
      // 构建注入消息列表 - 使用按角色分组的注入方式
      const promptInjects = sectionsToInjects(promptSections, 4, 'in_chat')
      const injects: Array<{
        content: string
        ...
        // 按角色分组的提示词注入（depth=4，最深）
        ...promptInjects,
      ]

      // 追加核心状态和游戏存档作为 system 消息
      const stateInject = `${coreStatusSummary}
${vectorMemorySection ? `\n${vectorMemorySection}\n` : ''}
# 游戏状态
你正在修仙世界《仙途》中扮演GM。以下是当前完整游戏存档(JSON格式):
${stateToonString}`.trim()
      if (stateInject) {
        injects.push({
          content: stateInject,
          role: 'system',
      ...
      const memoryToSend = (
        typeof shortTermMemoryForPrompt !== 'undefined' ? shortTermMemoryForPrompt : shortTermMemory
      ) as string[]
...
- 如果是首次穿越，要描写穿越的过程（空间扭曲、虚空通道等）`,
      ...
      console.log(
        `[AI双向系统] shouldActuallySplit=${shouldActuallySplit}, isSplitEnabled=${isSplitEnabled}, tavernHelper=${!!tavernHelper}`,
      )
      ...
      if (shouldActuallySplit) {
        ...
        const buildSplitSystemPrompt = async (step: 1 | 2): Promise<string> => {
          ...
          // 获取自创提示词内容（已启用的 coreRequest 自创提示词）
          const { promptStorage } = await import('@/services/promptStorage')
          const allPrompts = await promptStorage.loadAll()
          const customSections = Object.entries(allPrompts)
            .filter(([_, p]) => p.isCustom && p.enabled && p.category === 'coreRequest')
            .sort(
              (a, b) =>
                (a[1].customOrder ?? a[1].order ?? 999) - (b[1].customOrder ?? b[1].order ?? 999),
            )
...
`.trim()
          ...
          const sanitizedBusinessRulesPrompt = tavernEnv
            ? businessRulesPrompt
            : stripNsfwContent(businessRulesPrompt)
          sections.push(
          ...
          if (actionOptionsEnabled) {
            ...
            const customPromptSection = uiStore.actionOptionsPrompt
              ...
              : '（无特殊要求，按默认规则生成）'
            sections.push(
              actionOptionsPrompt.replace('{{CUSTOM_ACTION_PROMPT}}', customPromptSection),
            )
          }
...
`.trim()
        ...
        const buildSplitInjects = (
          ...
          // 🔥 只在第1步注入短期记忆，避免重复
          const memoryToSend = (
            typeof shortTermMemoryForPrompt !== 'undefined'
              ? shortTermMemoryForPrompt
              : shortTermMemory
          ) as string[]
...
`.trim()
        ...
        for (let attempt = 1; attempt <= 2; attempt++) {
          try {
            ...
            parsedStep2 = this.parseAIResponse(
      ...
      if (!isSplitEnabled) {
        ...
        try {
          ...
          console.warn(
            '[AI双向系统] 使用容错模式提取内容 - 文本长度:',
            extractedText.length,
            '记忆:',
            extractedMemory.length,
            '指令数:',
            extractedCommands.length,
            '行动选项:',
            extractedActionOptions.length,
          )
        }
      }
    ...
    try {
      ...
      const dataForProcessing = isSaveDataV3(saveData)
        ? saveData
        : migrateSaveDataToLatest(saveData).migrated
      const {
      ...
      if (!onlineLogPosted) {
        try {
          ...
          if (inTravel && Number.isFinite(sessionId) && sessionId > 0) {
            ...
            const allChanges = Array.isArray((stateChanges as any)?.changes)
              ? ((stateChanges as any).changes as any[])
              : []
            const relevantChanges = allChanges.filter((c) => {
              const key = String(c?.key ?? '')
              if (!key) return false
              if (key.startsWith('系统.历史') || key.startsWith('历史.')) return false
              if (key.includes('系统.历史') || key.includes('叙事历史') || key.includes('对话历史'))
                return false
              return true
            })
...
- 严禁使用占位文本或照抄示例`)
        ...
        const generateOnce = async (args: {
          ...
          user: string
          should_stream: boolean
          usageType?: InitialSplitUsageType
          onStreamChunk?: (chunk: string) => void
        }): Promise<string> => {
...
请按"分步生成（开局-第2步）"规则输出 JSON。
        ...
        // 🔥 第2步指令生成：可单独控制是否流式（部分API不支持流式）
        const step2StreamingInitial =
          !!apiStore.aiGenerationSettings?.splitStep2Streaming && useStreaming
        const initStep2UsageType: APIUsageType = hasInstructionApi
        ...
        gmResponse = {
          ...
          action_options: actionOptionsEnabled
            ? this.sanitizeActionOptionsForDisplay(
                parsedStep2.action_options?.length
                  ? parsedStep2.action_options
                  : defaultInitialActionOptions,
              )
            : [],
        }
      ...
      if (!shouldActuallySplit) {
        ...
        if (!response || String(response).trim().length === 0) {
          throw new Error(
            'AI返回了空响应。可能原因：1) 模型使用了reasoning_content字段而非content字段（如Gemini 3 Pro）；2) API配置错误；3) 网络问题。建议：在酒馆设置中关闭流式传输，或更换模型。',
          )
        }
  ...
  public async processGmResponse(
    ...
    const behavior = {
      ...
      implicitMidFallbackMaxLen:
        typeof options?.implicitMidFallbackMaxLen === 'number' &&
        Number.isFinite(options.implicitMidFallbackMaxLen)
    ...
    if (textContent) {
      ...
      if (behavior.appendShortTermMemoryFromText) {
        ...
        if (!(saveData as any).社交.记忆)
          (saveData as any).社交.记忆 = {
            短期记忆: [],
            中期记忆: [],
            长期记忆: [],
            隐式中期记忆: [],
          }
        if (!Array.isArray((saveData as any).社交.记忆.短期记忆))
          (saveData as any).社交.记忆.短期记忆 = []
    ...
    if (behavior.appendImplicitMidMemoryFromMidTerm && midTermContent) {
      if (!(saveData as any).社交) (saveData as any).社交 = {}
      if (!(saveData as any).社交.记忆)
        (saveData as any).社交.记忆 = { 短期记忆: [], 中期记忆: [], 长期记忆: [], 隐式中期记忆: [] }
      if (!Array.isArray((saveData as any).社交.记忆.隐式中期记忆))
        (saveData as any).社交.记忆.隐式中期记忆 = []
    ...
    if (
      ...
      if (!Array.isArray((saveData as any).社交.记忆.隐式中期记忆))
        (saveData as any).社交.记忆.隐式中期记忆 = []
      const fallback =
        textContent.length > behavior.implicitMidFallbackMaxLen
          ? `${textContent.slice(0, behavior.implicitMidFallbackMaxLen)}…`
          : textContent
    ...
    try {
      ...
      if (memorySettings) {
        ...
        const limit =
          typeof settings.shortTermLimit === 'number' && settings.shortTermLimit > 0
            ? settings.shortTermLimit
            : typeof settings.maxShortTerm === 'number' && settings.maxShortTerm > 0
              ? settings.maxShortTerm
              : null
    ...
    while (
      (saveData as any).社交?.记忆?.短期记忆 &&
      (saveData as any).社交.记忆.短期记忆.length > SHORT_TERM_LIMIT
    ) {
      // 删除最旧的短期记忆（第一个）
      ;(saveData as any).社交.记忆.短期记忆.shift()
      console.log(
        `[AI双向系统] 短期记忆超过上限（${SHORT_TERM_LIMIT}条），已删除最旧的短期记忆。当前短期记忆数量: ${(saveData as any).社交.记忆.短期记忆.length}`,
      )

      // 将对应的隐式中期记忆转化为正式中期记忆
      if (
        (saveData as any).社交.记忆.隐式中期记忆 &&
        (saveData as any).社交.记忆.隐式中期记忆.length > 0
      ) {
        const implicitMidTerm = (saveData as any).社交.记忆.隐式中期记忆.shift()
        if (implicitMidTerm) {
          if (!Array.isArray((saveData as any).社交.记忆.中期记忆))
            (saveData as any).社交.记忆.中期记忆 = []
          ;(saveData as any).社交.记忆.中期记忆.push(implicitMidTerm)
          console.log(
            `[AI双向系统] 已将隐式中期记忆转化为正式中期记忆。当前中期记忆数量: ${(saveData as any).社交.记忆.中期记忆.length}`,
          )
        }
      }
    }
    ...
    try {
      ...
      if (
        (saveData as any).社交?.记忆?.中期记忆 &&
        (saveData as any).社交.记忆.中期记忆.length >= midTermTrigger
      ) {
    ...
    if (protectionMode === 'skeleton') {
      ...
      const isV3Path = (p: string) =>
        allowedRoots.some((root) => p === root || p.startsWith(`${root}.`))
      ...
      preprocessedCommands.forEach((cmd, index) => {
        ...
        if (!validActions.has(action)) {
          rejectedCommands.push({
            command: cmd,
            errors: [`指令${index}: action 无效（${action}）`],
          })
          return
        }
        if (!key) {
        ...
        if (!isV3Path(key)) {
          rejectedCommands.push({
            command: cmd,
            errors: [`指令${index}: key 必须以 元数据/角色/社交/世界/系统 开头（当前: ${key}）`],
          })
          return
        }
        if (forbiddenPrefixes.some((p) => key === p || key.startsWith(`${p}.`))) {
          rejectedCommands.push({
            command: cmd,
            errors: [`指令${index}: 禁止AI直接操作系统自动字段（${key}）`],
          })
          return
        }
        if (action === 'delete' && allowedRoots.includes(key as any)) {
          rejectedCommands.push({
            command: cmd,
            errors: [`指令${index}: 禁止删除存档骨干根节点（${key}）`],
          })
          return
        }
    ...
    console.log(
      `[AI双向系统] 执行 ${sortedCommands.length} 条有效指令，拒绝 ${rejectedCommands.length} 条无效指令`,
    )
    ...
    const applyMode = (() => {
      try {
        ...
        if (raw === 'atomic' || raw === 'atomic_on_invalid_state' || raw === 'best_effort')
          return raw
      } catch {
        /* noop */
      }
    ...
    if (removedEffects.length > 0) {
      console.log(
        `[AI双向系统] Pinia状态更新前: 移除了 ${removedEffects.length} 个过期效果: ${removedEffects.join(', ')}`,
      )
    }
    ...
    if ((saveData as any).系统?.历史?.叙事 && (saveData as any).系统.历史.叙事.length > 0) {
      const latestNarrative = (saveData as any).系统.历史.叙事[
        (saveData as any).系统.历史.叙事.length - 1
      ]
    ...
    try {
      if (!(saveData as any).系统?.联机 || (saveData as any).系统?.联机?.模式 !== '联机') {
        const currentSectName = String(
          (saveData as any)?.社交?.宗门?.成员信息?.宗门名称 || '',
        ).trim()
        if (!currentSectName) {
          ...
          if (playerName && Array.isArray(factions)) {
            ...
            if (
              ownedSect &&
              typeof ownedSect === 'object' &&
              typeof ownedSect.名称 === 'string' &&
              ownedSect.名称.trim()
            ) {
              const { createJoinedSectState } = await import('@/utils/sectSystemFactory')
              const { sectSystem, memberInfo } = createJoinedSectState(ownedSect, {
  ...
  public async triggerMemorySummary(options?: MemorySummaryOptions): Promise<void> {
    ...
    try {
      ...
      if (midTermMemories.length < midTermTrigger) {
        console.log(
          `[AI双向系统] 中期记忆数量(${midTermMemories.length})未达到触发阈值(${midTermTrigger})，取消总结。`,
        )
        toast.info(`中期记忆未达到触发阈值(${midTermTrigger}条)，已取消总结`, {
          id: 'memory-summary',
        })
        return
      }
      ...
      if (midTermMemories.length < numToSummarize) {
        console.log(
          `[AI双向系统] 中期记忆数量(${midTermMemories.length})不足以总结${numToSummarize}条，取消总结。`,
        )
      ...
      console.log(
        `[AI双向系统] 准备总结：从${midTermMemories.length}条中期记忆中，总结最旧的${numToSummarize}条，保留最新的${memoriesToKeep.length}条`,
      )
      console.log(
        `[AI双向系统] 配置：触发阈值=${midTermTrigger}, 保留数量=${midTermKeep}, 总结数量=${numToSummarize}`,
      )
      ...
      console.log(
        `[AI双向系统] 记忆总结模式: ${useRawMode ? 'Raw模式（纯净总结）' : '标准模式（带预设）'}, 传输方式: ${useStreaming ? '流式' : '非流式'}`,
      )
        ...
        if (useRawMode) {
          ...
          const rawResponse = await tavernHelper.generateRaw({
            ordered_prompts: [
              ...
              {
                role: 'user',
                content: ['Continue.', 'Proceed.', 'Next.', 'Go on.', 'Resume.'][
...
${saveDataJson}`
      ...
      console.log(
        `[AI双向系统] ✅ 总结完成：${numToSummarize}条中期记忆 -> 1条长期记忆。保留 ${memoriesToKeep.length} 条。`,
      )
  ...
  private _preprocessCommands(commands: any[]): any[] {
    ...
    const normalizeCommandKey = (key: unknown): unknown => {
      ...
      if (trimmed === '角色.声望' || trimmed.startsWith('角色.声望.'))
        return trimmed.replace(/^角色\.声望/, '角色.属性.声望')
      if (trimmed === '角色.气血' || trimmed.startsWith('角色.气血.'))
        return trimmed.replace(/^角色\.气血/, '角色.属性.气血')
      if (trimmed === '角色.灵气' || trimmed.startsWith('角色.灵气.'))
        return trimmed.replace(/^角色\.灵气/, '角色.属性.灵气')
      if (trimmed === '角色.神识' || trimmed.startsWith('角色.神识.'))
        return trimmed.replace(/^角色\.神识/, '角色.属性.神识')
      if (trimmed === '角色.寿命' || trimmed.startsWith('角色.寿命.'))
        return trimmed.replace(/^角色\.寿命/, '角色.属性.寿命')
      if (trimmed === '角色.境界' || trimmed.startsWith('角色.境界.'))
        return trimmed.replace(/^角色\.境界/, '角色.属性.境界')
      if (trimmed === '角色.状态效果' || trimmed.startsWith('角色.状态效果'))
        return trimmed.replace(/^角色\.状态效果/, '角色.效果')
      ...
      if (trimmed.startsWith('关系矩阵.'))
        return `社交.关系矩阵.${trimmed.slice('关系矩阵.'.length)}`
      ...
      if (trimmed.startsWith('叙事历史.'))
        return `系统.历史.叙事.${trimmed.slice('叙事历史.'.length)}`
      ...
      if (trimmed === '物品栏' || trimmed.startsWith('物品栏.'))
        return `角色.背包.${trimmed.slice('物品栏.'.length)}`
    ...
    const expandLegacySetState = (cmd: any): any[] | null => {
      ...
      console.warn(
        `[AI双向系统] 预处理: 发现旧指令格式 set_state，已转换为 ${entries.length} 条 set 指令。`,
      )
    ...
    const expandNpcWholeSet = (cmd: any): any[] | null => {
      ...
      if (expanded.length > 0) {
        console.warn(
          `[AI双向系统] 预处理: 将整体 set "${key}" 拆分为 ${expanded.length} 条字段 set，避免覆盖丢字段。`,
        )
      }
      return expanded
    }
    ...
    const expandSocialRelationsWholeSet = (cmd: any): any[] | null => {
      ...
      for (const [npcName, npcValue] of Object.entries(value as Record<string, any>)) {
        ...
        const expandedNpc = expandNpcWholeSet({
          action: 'set',
          key: `社交.关系.${npcName}`,
          value: npcValue,
        })
      ...
      if (expanded.length > 0) {
        console.warn(
          `[AI双向系统] 预处理: 将整体 set "社交.关系" 拆分为 ${expanded.length} 条 NPC set，避免被保护拒绝。`,
        )
      }
      return expanded
    }
  ...
  private _repairTechniqueItem(item: any): any {
    ...
    const techniqueName =
      typeof repaired.名称 === 'string' && repaired.名称.trim() ? repaired.名称.trim() : '未知功法'
    ...
    if (!repaired.品质 || typeof repaired.品质 !== 'object') {
      ...
      if (typeof repaired.品质.quality !== 'string' || !repaired.品质.quality.trim())
        repaired.品质.quality = '凡'
      if (typeof repaired.品质.grade !== 'number' || !Number.isFinite(repaired.品质.grade))
        repaired.品质.grade = 0
    }
    ...
    const progress =
      typeof repaired.修炼进度 === 'number' && Number.isFinite(repaired.修炼进度)
        ? repaired.修炼进度
        : 0
    ...
    repaired.功法技能 = repaired.功法技能
      ...
      .map((s: any, idx: number) => {
        const skillName =
          typeof s.技能名称 === 'string' && s.技能名称.trim()
            ? s.技能名称.trim()
    ...
    if (repaired.功法技能.length === 0) {
      console.warn(
        `[AI双向系统] 预处理: 功法 "${techniqueName}" 缺少功法技能，已自动补齐基础技能以防报错。`,
      )
      repaired.功法技能 = [
        {
    ...
    for (const s of repaired.功法技能) {
      ...
      if (
        ...
        typeof s.技能名称 === 'string' &&
        !repaired.已解锁技能.includes(s.技能名称)
      ) {
  ...
  private executeCommand(
    ...
    const playerName =
      typeof (saveData as any)?.角色?.身份?.名字 === 'string'
        ? (saveData as any).角色.身份.名字.trim()
        : ''
    if (playerName) {
      const segments = path.split('.')
      const npcKey = typeof segments[2] === 'string' ? segments[2].trim() : ''
      const isPlayerInRelations =
        segments[0] === '社交' && segments[1] === '关系' && npcKey === playerName
      ...
      if (!playerName || npcName !== playerName) {
        ...
        if (protectionMode === 'strict') {
          ...
          if (!isPlainObject(existingNpc)) {
            ...
            const name =
              typeof (existingNpc as any).名字 === 'string' ? (existingNpc as any).名字.trim() : ''
            if (!name) {
              const [ok, repaired] = validateAndRepairNpcProfile(
                { ...(existingNpc as any), 名字: npcName },
                gameTime,
              )
              if (ok && repaired) set(saveData, npcRootPath, repaired)
            }
    ...
    if (action === 'set') {
      ...
      if (isNpcRoot && !isPlainObject(value)) {
        console.warn(
          `[AI双向系统] 阻止将 NPC 根对象 "${path}" set 为非对象值（${typeof value}），请改为设置具体字段。`,
        )
        return
      }
      if (isNpcRoot && isPlainObject(value)) {
        if (
          playerName &&
          typeof (value as any).名字 === 'string' &&
          (value as any).名字.trim() === playerName
        ) {
          console.warn(`[AI双向系统] 阻止将玩家本人写入社交.关系: ${path}`)
          return
        }
    ...
    switch (action) {
      ...
      case 'add': {
        ...
        if (path.includes('灵石') && newValue < 0) {
          console.warn(
            `[AI双向系统] ${path} 执行add后会变成负数 (${currentValue} + ${value} = ${newValue})，已限制为0`,
          )
          set(saveData, path, 0)
        } else {
      ...
      case 'push': {
        ...
        const isMemoryPath = path.startsWith('社交.记忆.') || path.startsWith('记忆.')
        const isImplicitMid = path === '社交.记忆.隐式中期记忆' || path === '记忆.隐式中期记忆'
      ...
      case 'pull': {
        ...
        console.log(
          `[AI双向系统] PULL操作: 从 ${path} 移除了 ${array.length - updatedArray.length} 个元素`,
        )
        break
      }
  ...
  private _summarizeValueForChangeLog(key: string, value: any, action: string): any {
    ...
    // 🔥 关键路径：短期记忆、中期记忆
    if (
      key.includes('记忆.短期记忆') ||
      key.includes('记忆.中期记忆') ||
      key.includes('记忆.隐式中期记忆')
    ) {
      if (Array.isArray(value) && value.length > 0) {
        return {
          __type: 'Array',
  ...
  private parseAIResponse(
    ...
    const standardize = (obj: Record<string, unknown>): GM_Response => {
      const commands = Array.isArray(obj.tavern_commands)
        ? obj.tavern_commands
        : Array.isArray(obj.指令)
          ? obj.指令
      ...
      if (enableActionOptions) {
        actionOptions = Array.isArray(obj.action_options)
          ? obj.action_options
          : Array.isArray(obj.行动选项)
            ? obj.行动选项
            : []
      }

      const filtered = (Array.isArray(actionOptions) ? actionOptions : []).filter(
