...
function normalizeRelationshipMatrixV3(raw: unknown, npcNames: string[]): any | null {
  ...
  const nodes = Array.isArray(matrix.nodes)
    ? matrix.nodes.map((n: any) => (typeof n === 'string' ? n.trim() : '')).filter(Boolean)
    : []
  ...
  for (const e of edgesRaw) {
    ...
    const score =
      typeof (e as any).score === 'number' && Number.isFinite((e as any).score)
        ? (e as any).score
        : undefined
  ...
  return {
    version:
      typeof matrix.version === 'number' && Number.isFinite(matrix.version) ? matrix.version : 1,
  ...
  actions: {
    ...
    async loadGame(characterId: string, saveSlot: string) {
      ...
      if (saveData) {
        ...
        console.error(
          `[GameState] No save data found for character ${characterId}, slot ${saveSlot}`,
        )
      }
    },
    ...
    loadFromSaveData(saveData: SaveData) {
      const v3 = (
        isSaveDataV3(saveData) ? saveData : migrateSaveDataToLatest(saveData).migrated
      ) as any
      ...
      const normalizeQualitySuffix = (obj: any, field: string) => {
        ...
        if (typeof raw === 'object') {
          const qualityName = String(
            (raw as any).quality ?? (raw as any).品质 ?? (raw as any).品阶 ?? '',
          )
      ...
      const relationships: Record<string, NpcProfile> | null = v3?.社交?.关系
        ? deepCopy(v3.社交.关系)
        : null
      const relationshipMatrix = normalizeRelationshipMatrixV3(
        v3?.社交?.关系矩阵,
      ...
      let sectMemberInfo: SectMemberInfo | null = (v3?.社交?.宗门 as any)?.成员信息
        ? deepCopy((v3.社交.宗门 as any).成员信息)
        : null
      ...
      try {
        if (!sectMemberInfo) {
          ...
          const sectNameCandidate = String(
            (sectSystem as any)?.当前宗门 || leader.sectName || '',
          ).trim()
          if (sectNameCandidate) {
            const sectProfile =
              factions.find((s) => String((s as any)?.名称 || '').trim() === sectNameCandidate) ??
              null
            sectMemberInfo = {
      ...
      let bodyPartDevelopment =
        body && typeof body === 'object' && (body as any).部位开发
          ? deepCopy((body as any).部位开发)
          : null
      ...
      if (isTavernEnv()) {
        const bodyObj: Record<string, any> = body && typeof body === 'object' ? deepCopy(body) : {}
    ...
    toSaveData(): SaveData | null {
      ...
      if (missingFields.length > 0) {
        console.error(
          '[gameStateStore.toSaveData] 存档数据不完整，缺少以下字段:',
          missingFields.join(', '),
        )
      ...
      const daoNormalized =
        this.thousandDao &&
        typeof this.thousandDao === 'object' &&
        (this.thousandDao as any).大道列表
          ? this.thousandDao
          : { 大道列表: {} }
      ...
      const sectNormalized =
        this.sectSystem || this.sectMemberInfo
          ? {
              ...(this.sectSystem || {}),
              ...(this.sectMemberInfo ? { 成员信息: this.sectMemberInfo } : {}),
            }
          : null
    ...
    updateState(path: string, value: any) {
      ...
      if (currentRoot && typeof currentRoot === 'object') {
        ...
        console.log(
          `[gameStateStore] ✅ 已更新 ${path} = ${JSON.stringify(value).substring(0, 100)}`,
        )
      } else {
    ...
    addToShortTermMemory(content: string) {
      ...
      const hasTimePrefix =
        content.startsWith('【仙道') ||
        content.startsWith('【未知时间】') ||
        content.startsWith('【仙历')
      ...
      const maxShortTerm = (() => {
        try {
          ...
          const limit =
            typeof parsed.shortTermLimit === 'number' && parsed.shortTermLimit > 0
              ? parsed.shortTermLimit
              : typeof parsed.maxShortTerm === 'number' && parsed.maxShortTerm > 0
                ? parsed.maxShortTerm
                : 5
          return limit
        } catch {
    ...
    getRegionMap(locationId: string) {
      const maps = (this.worldInfo as any)?.区域地图 as
        | import('@/types/gameMap').RegionMap[]
        | undefined
    ...
    addWorldLocation(location: {
      ...
      console.log(
        `[gameStateStore] ✅ 已添加新地点: ${location.名称} (${location.坐标.x}, ${location.坐标.y})`,
      )
    },
    ...
    addWorldLocationToRealm(
      ...
      if (exists) {
        console.warn(
          `[gameStateStore] 境界地图 "${realmKey}" 中地点 "${location.名称}" 已存在，跳过`,
        )
        return
      }
