...
export const useUIStore = defineStore('ui', () => {
  ...
  // 🔥 保存流式阶段已显示的正文内容（供最终叙事复用，避免切换页面后丢失）
  ...
  // --- 新增：数据验证错误弹窗方法 ---
  function showDataValidationErrorDialog(
    messages: string[],
    onConfirm: () => void,
    context: 'creation' | 'loading' = 'creation',
  ) {
  ...
  return {
    ...
    // 🔥 流式完成后的正文内容（跨页面持久化）
    lastStreamedContent,
    setLastStreamedContent: (text: string) => {
      lastStreamedContent.value = text
    },
    clearLastStreamedContent: () => {
      lastStreamedContent.value = ''
    },
