...
import {
  updateLifespanFromGameTime,
  updateNpcLifespanFromGameTime,
} from '@/utils/lifespanCalculator' // <-- 导入寿命计算工具
...
import {
  detectLegacySaveData,
  isSaveDataV3,
  migrateSaveDataToLatest,
  extractSaveDisplayInfo,
...
function filterSaveDataForCloud(saveData: SaveData | null): SaveData | null {
  ...
  if (
    anyFiltered?.系统?.历史 &&
    typeof anyFiltered.系统.历史 === 'object' &&
    '叙事' in anyFiltered.系统.历史
  ) {
...
export const useCharacterStore = defineStore('characterV3', () => {
    ...
    try {
      ...
      Object.entries(rootState.value.角色列表).forEach(([charId, profile]) => {
        ...
        if (
          anyProfile.模式 === '联机' &&
          anyProfile.存档列表?.['存档'] &&
          !anyProfile.存档列表?.['云端修行']
        ) {
          anyProfile.存档列表['云端修行'] = {
            ...(anyProfile.存档列表['存档'] as any),
            存档名: '云端修行',
          }
        ...
        if (
          profile.模式 === '单机' &&
          profile.存档 &&
          (!profile.存档列表 || Object.keys(profile.存档列表).length === 0)
        ) {
  ...
  const syncRootStateToCloud = async (): Promise<void> => {
    debug.log(
      '角色商店',
      'syncRootStateToCloud called. (Placeholder - no backend implementation yet)',
    )
  ...
  const createNewCharacter = async (
  ...
  ): Promise<CharacterBaseInfo | undefined> => {
    ...
    try {
      ...
      try {
        console.log('[角色商店] 调用initializeCharacter,参数:', {
          charId,
          baseInfo: authoritativeBaseInfo,
          world: world.name,
          age,
          useStreaming: creationStore.useStreamingStart,
          generateMode: creationStore.generateMode,
          splitResponseGeneration: creationStore.splitResponseGeneration,
        })
        initialSaveData = await initializeCharacter(
          charId,
          authoritativeBaseInfo,
          world,
          age,
          creationStore.useStreamingStart,
          creationStore.generateMode,
  ...
  const loadGame = async (charId: string, slotKey: string) => {
          ...
          if (cloudSaveData) {
            ...
            if (currentOnlineSlot) {
              currentOnlineSlot.云端同步信息 = {
                最后同步: cloudSave?.last_sync
                  ? String(cloudSave.last_sync)
                  : new Date().toISOString(),
                版本:
                  typeof cloudSave?.version === 'number'
                    ? cloudSave.version
                    : (currentOnlineSlot.云端同步信息?.版本 ?? 1),
                需要同步: false,
                后端创建失败: false,
              }
              await commitMetadataToStorage()
    ...
    if (!targetSlot.存档数据) {
      ...
      try {
        ...
        if (saveData) {
          console.log(
            '[15] 从IndexedDB加载的角色.背包.灵石数据:',
            (saveData as any).角色?.背包?.灵石,
          )
    ...
    try {
      ...
      const gameStateStore = useGameStateStore()
      // 🔥 先重置 gameStateStore，防止旧存档数据残留
      gameStateStore.resetState()
      if (targetSlot.存档数据) {
        const patched = isTavernEnv()
          ? (ensureSaveDataHasTavernNsfw(targetSlot.存档数据) as SaveData)
          : targetSlot.存档数据
          ...
          if (vectorMemoryService.isEnabled()) {
            ...
            if (existingMemories.length > 0) {
              ...
              if (stats.total === 0) {
                debug.log(
                  '角色商店',
                  `向量记忆库为空，开始导入 ${existingMemories.length} 条长期记忆`,
                )
  ...
  const syncFromIndexedDB = async () => {
    ...
    try {
      ...
      if (
        localNarrativeHistory &&
        Array.isArray(localNarrativeHistory) &&
        localNarrativeHistory.length > 0
      ) {
        ...
        debug.log(
          '角色商店',
          `[同步] 保留本地叙事历史数据 (${localNarrativeHistory.length}条)，避免被酒馆旧数据覆盖`,
        )
      }
      ...
      debug.log(
        '角色商店',
        `最终背包物品数量: ${Object.keys((saveData as any).角色?.背包?.物品 || {}).length}`,
      )
  ...
  const saveCurrentGame = async (options?: { notifyIfNoActive?: boolean }) => {
    ...
    try {
      ...
      console.log(
        '[11] toSaveData()返回的角色.背包.灵石数据:',
        (currentSaveData as any).角色?.背包?.灵石,
      )
      ...
      debug.log(
        '角色商店',
        `✅ 存档内容已保存到 IndexedDB (Key: ${active.角色ID}_${active.存档槽位})`,
      )
  ...
  const copySave = async (
    ...
    try {
      ...
      if (playerAttributes?.境界 && playerAttributes.境界.下一级所需 > 0) {
        newSlot.修为进度 = Math.floor(
          (playerAttributes.境界.当前进度 / playerAttributes.境界.下一级所需) * 100,
        )
      }
  ...
  const updateCharacterData = async (stateChanges: StateChangeLog) => {
    ...
    if (changedPaths.length > 0) {
      ...
      debug.log(
        '角色商店',
        `✅ 角色数据已更新并增量同步 ${changedPaths.length} 个字段`,
        changedPaths,
      )
    }
  }
  ...
  const importSave = async (charId: string, saveData: SaveSlot) => {
    ...
    if (saveData.存档数据) {
      const patched = isTavernEnv()
        ? (ensureSaveDataHasTavernNsfw(saveData.存档数据) as any)
        : saveData.存档数据
      const v3Data = (
        isSaveDataV3(patched as any) ? patched : migrateSaveDataToLatest(patched as any).migrated
      ) as any
   ...
   * @param commands 指令数组
   */
  const executeTavernCommands = async (
    saveData: SaveData,
    ...
    const setNestedValue = (
      obj: Record<string, unknown> | SaveData | CharacterProfile,
      path: string,
      value: unknown,
    ) => {
  ...
  const deleteNpc = async (npcName: string) => {
    ...
    try {
      ...
      console.log(
        `[删除NPC-调试] 删除前 gameStateStore.relationships 的引用:`,
        gameStateStore.relationships === relationships,
      )
      console.log(
        `[删除NPC-调试] 删除前 relationships 中的NPC列表:`,
        Object.keys(relationships).map((k) => relationships[k]?.名字),
      )
      ...
      console.log(
        `[删除NPC-调试] 删除后 relationships 中的NPC数量:`,
        Object.keys(relationships).length,
      )
      console.log(`[删除NPC-调试] 删除后是否还存在该NPC:`, !!relationships[npcKey])
      console.log(
        `[删除NPC-调试] 删除后 gameStateStore.relationships 中是否还存在:`,
        !!gameStateStore.relationships?.[npcKey],
      )
      console.log(
        `[删除NPC-调试] 删除后 relationships 中的NPC列表:`,
        Object.keys(relationships).map((k) => relationships[k]?.名字),
      )
      ...
      if ((savedData as any)?.社交?.关系) {
        const npcStillExists = Object.values((savedData as any).社交.关系).some(
          (npc: any) => npc?.名字 === npcName,
        )
        console.log(`[删除NPC-调试] toSaveData() 返回的数据中是否还存在该NPC:`, npcStillExists)
        console.log(
          `[删除NPC-调试] toSaveData() 返回的关系数量:`,
          Object.keys((savedData as any).社交.关系).length,
        )
        console.log(
          `[删除NPC-调试] toSaveData() 返回的NPC列表:`,
          Object.values((savedData as any).社交.关系).map((npc: any) => npc?.名字),
        )
      }
  ...
  const equipTechnique = async (itemId: string) => {
    ...
    // 1. 卸下当前所有功法
    Object.values(((saveData as any).角色?.背包?.物品 ?? {}) as Record<string, Item>).forEach(
      (i) => {
        if (i.类型 === '功法') {
          i.已装备 = false
        }
    ...
    if (item.功法技能 && Array.isArray(item.功法技能)) {
      ...
      debug.log(
        '角色商店',
        `[技能解锁检查] 功法: ${item.名称}, 进度: ${currentProgress}%, 技能数: ${item.功法技能.length}`,
      )
      item.功法技能.forEach((skill: any) => {
        const unlockThreshold = skill.熟练度要求 || 0
        debug.log(
          '角色商店',
          `  检查技能: ${skill.技能名称}, 阈值: ${unlockThreshold}%, 当前进度: ${currentProgress}%, 应解锁: ${currentProgress >= unlockThreshold}`,
        )
      ...
      if (itemInStore.功法技能 && Array.isArray(itemInStore.功法技能)) {
        itemInStore.功法技能.forEach((skill: any) => {
          ...
          if (
            ...
            !itemInStore.已解锁技能!.includes(skill.技能名称)
          ) {
  ...
  const importCharacter = async (profileData: CharacterProfile & { _导入存档列表?: any[] }) => {
    ...
    if (importedSaves && Array.isArray(importedSaves) && importedSaves.length > 0) {
      ...
      for (const save of importedSaves) {
        ...
        if (save.存档数据) {
          const patched = isTavernEnv()
            ? (ensureSaveDataHasTavernNsfw(save.存档数据) as any)
            : save.存档数据
          const v3Data = (
            isSaveDataV3(patched as any)
              ? patched
              : migrateSaveDataToLatest(patched as any).migrated
          ) as any
          const validation = validateSaveDataV3(v3Data as any)
          if (!validation.isValid) {
    ...
    debug.log(
      '角色商店',
      `成功导入角色: ${characterName} (新ID: ${newCharId})，含 ${Object.keys(profileData.存档列表).length} 个存档`,
    )
